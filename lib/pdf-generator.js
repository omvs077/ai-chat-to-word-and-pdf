...
(function () {
  ...
  // Splits markdown-formatted text into {text, link} chunks via the shared
  // parseInlineRuns (the same function docx-generator.js already uses for
  // real hyperlinks) so PDF links become real clickable regions instead of
  // the old flattened 'label (url)' text. Bold/italic/code flags from
  // parseInlineRuns are dropped here - PDF paragraphs render at a single
  // weight already, a separate pre-existing limitation this change doesn't
  // touch - but each run's .text already has markdown syntax stripped
  // (asterisks/backticks removed) the same way the old stripInline() did.
  function linkChunks(text) {
    return parseInlineRuns(text).map(r => ({ text: r.text, link: r.link || null }));
  }
  ...
  async function generatePdf(ir) {
    ...
    // Accepts an array of {text, link} chunks (see linkChunks()) rather than
    // a plain string, so word-wrapping can carry link membership through to
    // drawLines/list without losing which words belong to which URL.
    function layoutText(chunks, style, fontSize, maxWidth) {
      ...
      for (const chunk of chunks) {
        ...
        for (const word of words) {
          ...
          const runs = splitRuns(word, style).map(r => ({ ...r, width: measure(r), link: chunk.link || null }));
    ...
    // Brand blue reused from the existing 'You' role-label color elsewhere
    // in this file, so link styling matches the rest of the document rather
    // than introducing a new one-off color.
    const LINK_COLOR = [42, 93, 176];
    ...
    function drawLines(lines, style, fontSize, color, xStart) {
      ...
      for (const line of lines) {
        ...
        for (const run of line) {
          ...
          doc.setTextColor(...(run.link ? LINK_COLOR : color));
          doc.text(run.text, x, y);
          if (run.link) {
            // Clickable region over the drawn text, plus a thin underline
            // for visual affordance - matches the 'Hyperlink' style already
            // used in docx-generator.js for the same link data.
            doc.link(x, y - fontSize * 0.85, run.width, fontSize * 1.1, { url: run.link });
            doc.setDrawColor(...LINK_COLOR);
            doc.setLineWidth(0.5);
            doc.line(x, y + 1.5, x + run.width, y + 1.5);
          }
          x += run.width;
        }
        y += LINE_H;
    ...
    // text: either a plain string (no links - title/labels/export line) or
    // an array of {text, link} chunks from linkChunks() (body text, headings,
    // list items - anything that may contain real markdown links).
    function paragraph(text, { size = 10.5, style = 'normal', color = [30, 30, 30], gapAfter = 8 } = {}) {
      const chunks = Array.isArray(text) ? text : [{ text, link: null }];
      const lines = layoutText(chunks, style, size, MAX_W);
    ...
    function heading(text, level) {
      paragraph(linkChunks(text), { size: level === 1 ? 15 : 12.5, style: 'bold', gapAfter: 6 });
    }

    function list(items, ordered) {
      items.forEach((item, idx) => {
        const prefix = ordered ? `${idx + 1}. ` : '\u2022  ';
        const lines = layoutText(linkChunks(item), 'normal', 10.5, MAX_W - 16);
        lines.forEach((line, li) => {
          ...
          for (const run of line) {
            ...
            doc.setTextColor(...(run.link ? LINK_COLOR : [30, 30, 30]));
            doc.text(run.text, x, y);
            if (run.link) {
              doc.link(x, y - 10.5 * 0.85, run.width, 10.5 * 1.1, { url: run.link });
              doc.setDrawColor(...LINK_COLOR);
              doc.setLineWidth(0.5);
              doc.line(x, y + 1.5, x + run.width, y + 1.5);
            }
    ...
    for (const turn of ir.turns) {
      ...
      y += 14;
      paragraph(turn.role === 'user' ? 'You' : (ir.assistantName || 'Claude'), {
        size: 11, style: 'bold',
      ...
      for (const block of parseMarkdownBlocks(turn.markdown)) {
        if (block.type === 'heading') heading(block.text, block.level);
          ...
          else paragraph(linkChunks(block.text));
        }
      }
    }
