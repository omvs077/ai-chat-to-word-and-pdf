...
// Real markup pasted from live DevTools inspection of an actual "generate
// an image, no text" turn - the exact bug reported: the whole turn,
// including the "ChatGPT said:" label, was missing from the export
// entirely. Root cause: this turn's <section> carries data-turn="assistant"
// but NO data-message-author-role anywhere in its subtree, unlike every
...
test('an image-only assistant turn (no data-message-author-role) is not missing from export (regression)', async () => {
  ...
  `;
  const result = await run(html, { scrollList: { scrollHeight: 2000, clientHeight: 400 } });
  assert.equal(result.turns.length, 2, 'the image-only turn must not vanish');
  assert.equal(result.turns[0].role, 'user');
  assert.equal(result.turns[1].role, 'assistant');
  // Decorative aria-hidden duplicates must be gone; the real image becomes
  // a clean text placeholder using its real alt text, not a dead link to a
